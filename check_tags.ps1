...
if ($VSCODIUM_ASSETS -eq "null" -or !$VSCODIUM_ASSETS) {
  ...
  $SYSTEM_SETUP = $WindowsAssets | Where-Object { $_.name.Contains("MrCodeSetup-$env:BUILDARCH") }
  $USER_SETUP = $WindowsAssets | Where-Object { $_.name.Contains("MrCodeUserSetup-$env:BUILDARCH") }
  if (!$SYSTEM_SETUP) {
